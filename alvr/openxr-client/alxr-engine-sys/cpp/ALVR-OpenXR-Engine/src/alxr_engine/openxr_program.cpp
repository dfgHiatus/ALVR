...
#undef CAT
    ...
    bool PollEyeTracker(ALXREyeTrackingInfo& ALXREyeTrackingInfo)
    {
        ...
        for (int eye = 0; eye < 2; ++eye) {
            if (eyeGazes.gaze[eye].isValid) {
                ALXREyeTrackingInfo.a = eyeGazes.gaze[eye].gazePose.position.x;
            }
        }
    ...
    // Called in rust
    bool PollFaceTracker(ALXRFaceTrackingInfo& ALXRFaceTrackingInfo)
    {
        if (faceTracker_ == XR_NULL_HANDLE)
            return false;
        
        // TODO Poll confidence, etc.
        for (uint32_t i = 0; i < XR_FACE_EXPRESSION_COUNT_FB; ++i) {
            ALXRFaceTrackingInfo.weights[i] = expressionWeights.weights[i];
        }
