...
bool alxr_on_eye_update(ALXREyeTrackingInfo& ALXREyeTrackingInfo)
{
    const auto programPtr = gProgram;
    if (programPtr == nullptr)
        return false;

    return programPtr->PollEyeTracker(ALXREyeTrackingInfo);
}

bool alxr_on_face_update(ALXRFaceTrackingInfo& ALXRFaceTrackingInfo)
{
    const auto programPtr = gProgram;
    if (programPtr == nullptr)
        return false;
    
    return programPtr->PollFaceTracker(ALXRFaceTrackingInfo);
}
